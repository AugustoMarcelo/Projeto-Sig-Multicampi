Ext.define('Packt.controller.security.Groups', {
	...
	onButtonClickDelete: function(button, e, options) {
		...
		if(store.getCount() >= 2 && record[0] && usersGrid.getStore().getCount() == 0) {
					...
					if(buttonId == 'yes') {
						Ext.Ajax.request({
							...
							success: function(conn, response, options, eOpts) {
								...
								if(result.success) {
									Packt.util.Alert.msg(translations.msgSuccess, translations.msgGroupDeleted);
									store.load();
									form.reset();
			...
			Ext.Msg.show({
				title: translations.msgWarning,
				msg: translations.msgGroupWarning,
				buttons: Ext.Msg.OK,
			...
			Ext.Msg.show({
				title: translations.msgWarning,
				msg: translations.msgGroupWarning2,
				buttons: Ext.Msg.OK,
				icon: Ext.Msg.WARNING
			});
		}
	...
	onButtonClickSave: function(button, e, options) {
		...
		if(formPanel.getForm().isValid()) {
			if(names.length == 0) {
				Ext.Msg.show({
					title: translations.msgWarning,
					msg: translations.msgGroupWarning3,
					buttons: Ext.Msg.OK,
					icon: Ext.Msg.WARNING
				});
				...
				Ext.get(formPanel.getEl()).mask(translations.msgSaveWait, 'loading');
				...
				Ext.Ajax.request({
					...
					success: function(conn, response, options, eOpts) {
						...
						if(result.success) {
							Packt.util.Alert.msg(translations.msgSuccess, translations.msgGroupSave);
							store.load();
							formPanel.setDisabled(true);
