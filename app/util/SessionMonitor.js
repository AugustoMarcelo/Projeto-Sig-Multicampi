Ext.define('Packt.util.SessionMonitor', {
  ...
  window: Ext.create('Ext.window.Window', {
    ...
    modal: true,
    resizable: false,
    title: translations.sessionMsgTitle,
    width: 325,
    items: [{
      xtype: 'container',
      frame: true,
      html: translations.sessionMsg    
    },{
      xtype: 'label',
      text: ''
    }],
    buttons: [{
      text: translations.sessionButtonText1,
      handler: function() {
      ...
      text: translations.sessionButtonText2,
      action: 'logout',
      handler: function() {
        Ext.TaskManager.stop(Packt.util.SessionMonitor.countDownTask);
        Packt.util.SessionMonitor.window.hide();
   ...
   */
  countDown: function() {
    this.window.down('label').update(translations.sessionMsgTime1 + this.remaining + translations.sessionMsgTime2 + ((this.remaining == 1) ? '.' : 's.') );
